...
from constants import SPECIAL_PREFIX
...
class AMRDataset(Dataset):
	...
	def __init__(self, dataframe, tokenizer, src_max_len, tgt_max_len, \
				pretrained="t5", train=False, generation=True):
		self.tokenizer = tokenizer
		self.data = dataframe
	...
	def add_special_tokens(self, generation):
		...
		if self.pretrained.endswith("t5"):
			...
			for gsp in SPECIAL_PREFIX.values():
...
def process_data(src_path, tgt_path=None, prefix="lemma"):
	src = []
	with open(src_path, "r") as f:
		src = [SPECIAL_PREFIX[prefix] + " "+ line.strip() for line in f]
